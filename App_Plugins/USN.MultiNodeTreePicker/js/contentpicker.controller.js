...
function contentPickerController($scope, $http, entityResource, editorState, iconHelper, $routeParams, angularHelper, navigationService, $location, miniEditorHelper) {
    ...
    var defaultConfig = {
        ...
        startNode: {
            query: $scope.model.config.pickerLocation.type === "config" ? "$current/ancestor-or-self::USNWebsiteConfigurationSection/child::USNReusableContent[1]/child::" + $scope.model.config.pickerLocation.pickerStartDocType + "[1]" : "",
            type: "content",
            id: startNodeID
        }
    };
    ...
    if ($routeParams.section === "settings" && $routeParams.tree === "documentTypes") {
        ...
        if ($scope.model.config.pickerLocation.type === "content") {
            var promise = $http.get('backoffice/api/USNMultiPicker/GetID',
                {
                    params: {
                        currentNodeID: $routeParams.id,
                        homepageAlias: $scope.model.config.pickerLocation.homePageDocType,
                        configAlias: $scope.model.config.pickerLocation.websiteConfigPropertyAlias,
                        startAlias: $scope.model.config.pickerLocation.pickerStartDocType
                    }
                }
            );
...
angular.module('umbraco').controller("USN.ContentPickerController", contentPickerController);
